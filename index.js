import { examples } from "./examples/specs/specs-survey.js";
...
if (updateExampleOutput) {
    for (let i = 0; i < exampleNames.length; i ++) {
        ...
        const example = examples[exampleNames[i]]
        const textSpec = desc2text(gos2desc(example)).textLong

        
        //save output as its own text file
        fs.writeFile("./examples/output/example" + (i+1) + ".txt", textSpec, err => {
        if (err) {
            console.error(err);
        }
        });
